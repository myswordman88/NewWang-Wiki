// 暗金装备页面：搜索 + 分类/子类筛选 + 排序 + 译名对照 + 卡片网格
(function () {
  "use strict";

  var DATA = window.UNIQUE_ITEMS || [];
  ...
  function renderCard(r) {
    ...
    var tagline = '<div class="base-subtitle">' + tierBadge + dot + catBadge + '</div>';
    ...
    // 冠名赞助区块：插在【属性】与【需求等级】之间（仅 sponsored 标记的物品渲染）
    var sponsor = '';
    ...
    return '<article class="unique-card">'
      ...
      + '<h3 class="unique-name">' + esc(zhs) + '</h3>'
      + tagline
      + names
      + '</div>'
  ...
  // 暴露渲染函数：供 BD 构建页等第三方调用（须在本页或装载本文件的页面内使用）
  window.__uniqueCardRender = renderCard;
